    ...
    <div class="article-list">
        <el-form ref="formRef" :model="queryParams" :inline="true">
            <el-form-item label="文章名称">
                <el-input class="w-[280px]" v-model="queryParams.name" placeholder="请输入" clearable
        ...
        <!-- 表格 -->
        <el-table size="large" v-loading="pager.loading" :data="pager.lists" height="360px"
            @row-click="handleSelectItem">
            <el-table-column label="选择" min-width="50">
                <template #default="{ row }">
                    <div class="flex row-center">
                        <el-checkbox :model-value="getSelectItem(row.id)" size="large"
                            @change="handleSelectItem(row)"></el-checkbox>
                    </div>
                </template>
            </el-table-column>
